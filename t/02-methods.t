use Test::Most 0.25 tests => 4;

use Lymbix::API;

SKIP:{
    eval {require JSON};
    skip "Required to have JSON::XS installed and LYMBIX_AUTH_KEY env variable set", 4 unless $ENV{LYMBIX_AUTH_KEY} and not $@;
    use JSON;

    my $lymbix = Lymbix::API->new(
    ...
    subtest 'tonalize tests' => sub {
        ...
        eval {
            $res = decode_json $lymbix->tonalize($article);
        };
        ...
        eval {
            $res = decode_json $lymbix->tonalize($article, undef, 12345);
        };
        fail "Invalid response to decode. ERROR:[$@]" if $@;

        is $res->{reference_id}, 12345, 'Got expected value of reference_id in response';
    ...
    subtest 'tonalize_detailed tests' => sub {
        ...
        eval {
            $res = decode_json $lymbix->tonalize_detailed($article);
        };
        fail "Invalid response to decode. ERROR:[$@]" if $@;
    ...
    subtest 'tonalize_multiple tests' => sub {
        ...
        eval {
            $res = decode_json $lymbix->tonalize_multiple($articles, undef, '12345,54321');
        };
    ...
    subtest 'flag_response tests' => sub {
        ...
        my $res;
        my $phrase = "He was happy and surprised instead of being angry. Although he wasn't too happy about it he said yes anyways! What do you think of this decision?";

        dies_ok { $lymbix->flag_response(1234, $phrase, 'invalid_request') } 'Expecting to die on invalid api_method_request';
